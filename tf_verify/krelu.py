        ...
        if KAct.type == "ReLU":
            if approx:
                if use_wralu is None:
                    logging.critical("Using `fkrelu` solver.")
                    self.cons = fkrelu(self.input_hrep)
                elif use_wralu == "sci":
                    logging.critical("Using `krelu_with_sci` solver.")
                    self.cons = krelu_with_sci(self.input_hrep, lbi, ubi)
                elif use_wralu == "sciplus":
                    logging.critical("Using `krelu_with_sciplus` solver.")
                    self.cons = krelu_with_sciplus(self.input_hrep, lbi, ubi)
                elif use_wralu == "sciall":
                    logging.critical("Using `krelu_with_sciall` solver.")
