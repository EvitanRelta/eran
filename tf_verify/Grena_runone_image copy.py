...
def normalize_poly(
    num_params, lexpr_cst, lexpr_weights, lexpr_dim, uexpr_cst, uexpr_weights, uexpr_dim, means, stds, dataset
):
    # normalization taken out of the network
...
def model_predict(base, input):
    if is_onnx:
        ...
        pred = base.run(
            base.graph.get_operation_by_name(model.op.name).outputs[0],
...
def estimate_grads(specLB, specUB, dim_samples=3, input_shape=[1]):
    ...
    inputs = [
        (((dim_samples - i) * specLB + i * specUB) / dim_samples).reshape(*input_shape) for i in range(dim_samples + 1)
    ]
    ...
    for sample in range(dim_samples + 1):
        ...
        for index in range(len(specLB)):
            if sample < dim_samples:
                l_input = [
                    m if i != index else u for i, m, u in zip(range(len(specLB)), inputs[sample], inputs[sample + 1])
                ]
            ...
            if sample > 0:
                u_input = [
                    m if i != index else l for i, m, l in zip(range(len(specLB)), inputs[sample], inputs[sample - 1])
                ]
...
logging.basicConfig(
    filename=os.path.join(os.path.dirname(config.bounds_save_path), "log"),
    level=logging.CRITICAL,
    format=f"use_wralu={config.use_wralu}, eps={config.epsilon}, sparse_n={config.sparse_n}, k={config.k}, s={config.s} - %(message)s",
)
...
if is_saved_tf_model or is_pb_file:
    ...
    if is_onnx:
        ...
        model, is_conv, means, stds = read_tensorflow_net(
            netname, num_pixels, is_trained_with_pytorch, (domain == "gpupoly" or domain == "refinegpupoly")
        )
...
if not is_trained_with_pytorch:
    if dataset == "mnist" and not config.geometric:
        ...
        means = [1.9791091e04, 0.0, 0.0, 650.0, 600.0]
...
for i, test in enumerate(tests):
    ...
    if is_correctly_classified == True:
        ...
        if domain == "gpupoly" or domain == "refinegpupoly":
            ...
            if is_verified:
                ...
                is_verified, x = refine_gpupoly_results(
                    nn,
                    network,
                    num_gpu_layers,
                    relu_layers,
            ...
            if domain.endswith("poly"):
                ...
                perturbed_label, nn, nlb, nub, failed_labels, x = eran.analyze_box(
                    specLB,
                    specUB,
                    "deeppoly",
                    config.timeout_lp,
            ...
            if not domain.endswith("poly") or not (perturbed_label == label):
                ...
                perturbed_label, _, nlb, nub, failed_labels, x = eran.analyze_with_gt(
                    specLB,
                    specUB,
                    domain,
                    config.timeout_lp,
            ...
            if perturbed_label == label:
                ...
                if complete == True and failed_labels is not None:
                    ...
                    verified_flag, adv_image, adv_val = verify_network_with_milp(
    ...
    print(
        f"progress: {1 + i - config.from_test}/{config.num_tests}, "
